...
//        SVProgressHUDBackgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"spinner background"]];
...
//            SVProgressHUDForegroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"spinner background"]];
        }
...
- (void)layoutAnimatedLayer {
    ...
    layer.position = CGPointMake(self.bounds.size.width - layer.bounds.size.width / 1.5, self.bounds.size.height - layer.bounds.size.height / 1.5);
}

- (CAShapeLayer*)indefiniteAnimatedLayer {
    if(!_indefiniteAnimatedLayer) {
        CGPoint arcCenter = CGPointMake(self.radius+self.strokeThickness/1.5+5, self.radius+self.strokeThickness/1.5+5);
        CGRect rect = CGRectMake(0, 0, arcCenter.x*1.5, arcCenter.y*1.5);
        
        UIBezierPath* smoothedPath = [UIBezierPath bezierPathWithArcCenter:arcCenter
                                                                    radius:self.radius
                                                                startAngle:M_PI*3/1.5
                                                                  endAngle:M_PI/1.5+M_PI*5
                                                                 clockwise:YES];
        ...
        _indefiniteAnimatedLayer.lineWidth = rect.size.width;
        _indefiniteAnimatedLayer.lineCap = kCALineCapRound;
        _indefiniteAnimatedLayer.lineJoin = kCALineJoinBevel;
        _indefiniteAnimatedLayer.path = smoothedPath.CGPath;
        
        CALayer *maskLayer = [CALayer layer];
        maskLayer.contents = (id)[[UIImage imageNamed:@"loading"] CGImage];
